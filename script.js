...
(function () {
  ...
  if (photoViewer && typeof photoViewer.showModal === "function") {
    const photoFigures = Array.from(document.querySelectorAll(
    ...
    const viewerCount = document.getElementById("photoViewerCount");
    ...
    const photoCaption = (figure, image) => {
    ...
    const showPhoto = (index) => {
      activePhoto = (index + photoFigures.length) % photoFigures.length;
      const figure = photoFigures[activePhoto];
      const image = figure.querySelector("img");
      viewerImage.src = image.currentSrc || image.src;
      viewerImage.alt = image.alt;
      viewerCaption.textContent = photoCaption(figure, image);
      viewerCount.textContent = (activePhoto + 1) + " / " + photoFigures.length;
    };
    ...
    photoFigures.forEach((figure, index) => {
      ...
      figure.addEventListener("click", () => openPhoto(index));
    ...
    viewerPrev.addEventListener("click", () => showPhoto(activePhoto - 1));
    viewerNext.addEventListener("click", () => showPhoto(activePhoto + 1));
    ...
    photoViewer.addEventListener("keydown", (e) => {
      if (e.key === "ArrowLeft") showPhoto(activePhoto - 1);
      if (e.key === "ArrowRight") showPhoto(activePhoto + 1);
    });
